  ...
  return (
    <div className={`relative overflow-hidden rounded-xl ${sizeClasses[size]} ${className}`}>
      ...
      <div className="absolute bottom-0 inset-x-0 h-3/4">
        {/* Stadium Bowl */}
        <div className={`absolute bottom-0 inset-x-0 h-1/2 bg-gradient-to-t ${config.stadiumColor} rounded-t-3xl`}>
          {/* Stadium Lights */}
          <div className="absolute top-0 inset-x-0 flex justify-around">
            {Array.from({ length: 8 }, (_, i) => (
              <div key={i} className={`w-2 h-2 ${config.lightsColor} ${time.lightIntensity} rounded-full`}>
                💡
              </div>
            ))}
          </div>
          ...
          <div className="absolute top-4 inset-x-2 h-8 overflow-hidden">
            {Array.from({ length: Math.floor(crowdInfo.density / 5) }, (_, i) => (
              <div
                key={i}
        ...
        {/* Football Pitch */}
        <div className={`absolute bottom-0 inset-x-4 h-2/3 bg-gradient-to-br ${config.grassColor} rounded-lg`}>
          {/* Field Lines */}
          <div className="absolute inset-2 border-2 border-white/30 rounded">
            ...
            {/* Goals */}
            <div className="absolute top-1/2 -left-1 w-2 h-4 bg-white/60 transform -translate-y-1/2">🥅</div>
            <div className="absolute top-1/2 -right-1 w-2 h-4 bg-white/60 transform -translate-y-1/2">🥅</div>
          </div>
          
          {/* Field Shadows */}
          <div className={`absolute inset-0 bg-black/10 ${time.shadows} rounded-lg`}></div>
        </div>
      </div>
      ...
      <div className="absolute bottom-2 left-2 text-xs text-white/80 bg-black/50 rounded px-2 py-1">
        <div className="font-semibold">{config.atmosphere.toUpperCase()} ATMOSPHERE</div>
      ...
      {weather !== 'clear' && (
        <div className="absolute top-2 right-2 text-white/80 bg-black/50 rounded px-2 py-1 text-xs">
          {weather === 'rain' ? '🌧️' : weather === 'snow' ? '❄️' : '🌫️'} {weather}
        </div>
      )}
    </div>
  );
